...
(function () {
  ...
  const statuses = ["All statuses", "Answered", "Open"];
  ...
  function buildQueryParams() {
    const typeEl = document.querySelector(
      '.filterGroup input[name="type"]:checked'
    );
  ...
  function renderTickets(data) {
    ...
    const html = (data || [])
      .map((t) => {
        ...
        const vis =
          t && typeof t.is_Public !== "undefined"
            ? t.is_Public
              ? "public"
              : "private"
            : "public";
        ...
        return `
      <div class="ticket" tabindex="0" aria-label="Forum post ${esc(
        t.title
                ...
                <div class="ticketRow1">
                    ...
                    <div style="display:flex; align-items:center; gap:8px;">
                      <div class="status ${status.cls}">${esc(
          status.label
        )}</div>
                      <div class="status ${
                        vis === "public" ? "inProgress" : "rejected"
                      }" title="${vis === "public" ? "Public" : "Private"}">${
          vis === "public" ? "Public" : "Private"
            ...
            <div class="ticketDetail">
              ...
              <div class="ticketDataHolder votesHolder" data-id="${esc(t.id)}">
                <button type="button" class="voteBtn up" aria-label="Upvote post ${esc(
                  t.title
                )}">▲</button>
                <span class="voteUpCount">${esc(votesUp)}</span>
                <span style="display:inline-block; width:10px;"></span>
                <button type="button" class="voteBtn down" aria-label="Downvote post ${esc(
                  t.title
                )}">▼</button>
      ...
      container.addEventListener("click", (e) => {
        ...
        if (voteBtn) {
          ...
          if (voteBtn.classList.contains("up") && up) {
            ...
            down.textContent = String(
              parseInt(down.textContent || "0", 10) + 1
            );
          }
          return;
        }
  ...
  async function loadTickets(nextPage) {
    ...
    const container = document.querySelector(".tickets");
    if (container) {
      container.innerHTML = '<div class="ticketsLoading">Loading posts…</div>';
    }
    try {
      const p = buildQueryParams();
      ...
      const CACHE_KEY = `admin_forum_${qs.toString()}`;

      let forceBypass = false;
      try {
        if (localStorage.getItem("admin_forum_bust")) {
          forceBypass = true;
          localStorage.removeItem("admin_forum_bust");
        }
      } catch {}
      ...
      if (cached) {
        ...
        window.adminForumData = data;
        renderTickets(window.adminForumData);
        renderPagination();

        fetch(`/admin/forumData?${qs.toString()}`, { credentials: "include" })
          .then((res) =>
          ...
          .then((fresh) => {
            ...
            window.adminForumData = newData;
            renderTickets(window.adminForumData);
            renderPagination();
          })
          .catch((e) => {
            console.warn("Admin forum background refresh failed", e);
          });
        return;
      }

      const res = await fetch(`/admin/forumData?${qs.toString()}`, {
        credentials: "include",
      });
