// public/js/staff/staffFaqs.js
// This is a direct adaptation of adminFaqs.js, with ID prefixes changed to 'staff' for uniqueness.
// Place this in your public/js/staff/ directory.

(function () {
  const searchInput = document.getElementById("staffFaqSearch");
  const listEl = document.getElementById("staffFaqsList");
  const pagerEl = document.getElementById("staffFaqsPagination");
  const newFaqBtn = document.getElementById("staffNewFaqBtn");
  const deleteModal = document.getElementById("staffDeleteFaqModal");
  const cancelDeleteBtn = document.getElementById("staffCancelDeleteFaqBtn");
  const confirmDeleteBtn = document.getElementById("staffConfirmDeleteFaqBtn");
  const editModal = document.getElementById("staffEditFaqModal");
  const faqForm = document.getElementById("staffFaqForm");
  const faqQuestion = document.getElementById("staffFaqQuestion");
  const faqAnswer = document.getElementById("staffFaqAnswer");
  const cancelFaqEditBtn = document.getElementById("staffCancelFaqEditBtn");
  const editFaqModalTitle = document.getElementById("staffEditFaqModalTitle");
  let faqs = [];
  let page = 1;
  const perPage = 10;
  ...
  async function loadFaqs() {
    ...
    try {
      ...
      const res = await fetch(`/staff/faqsData?${qs.toString()}`, {
        credentials: "include",
      });
  ...
  confirmDeleteBtn?.addEventListener("click", async () => {
    ...
    try {
      await apiPost("/staff/faqDelete", { id: String(pendingDeleteId) });
  ...
  faqForm?.addEventListener("submit", async (e) => {
    ...
    try {
      if (editingId == null) {
        await apiPost("/staff/faqCreate", { question: q, answer: a });
      } else {
        await apiPost("/staff/faqUpdate", {
          id: String(editingId),
          question: q,
          answer: a,
        });
