(function(){
  ...
  toggleBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const val = btn.dataset.value || 'private';
  ...
  function addFiles(files){
    Array.from(files).forEach(f => buffer.items.add(f));
    syncInput();
    renderFiles();
  }
  ...
  function enhanceSelect(select){
    ...
    Array.from(select.options).forEach((opt, idx) => {
      ...
      li.addEventListener('click', () => {
        ...
        // Update label and selection states
    ...
    // Insert DOM: place wrapper after select and move select inside wrap (hidden)
