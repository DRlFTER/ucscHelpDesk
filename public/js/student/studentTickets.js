// Copied from admin/adminTickets.js for student tickets view; endpoints remain admin for now.
...
(function () {
  ...
  function openTicket(el) {
    ...
    const code = el.getAttribute("data-code");
      if (id) {
        window.location.assign(`/student/ticketFull?id=${encodeURIComponent(id)}`);
      } else if (code) {
        window.location.assign(`/student/ticketFull?code=${encodeURIComponent(code)}`);
    }
  }
  ...
  async function loadTickets(nextPage) {
    ...
    try {
      ...
      const CACHE_KEY = `student_tickets_${qs.toString()}`;
      ...
      if (cached) {
        ...
        fetch(`/student/ticketsData?${qs.toString()}`, { credentials: "include" })
      ...
      const res = await fetch(`/student/ticketsData?${qs.toString()}`, {
        credentials: "include",
      });
