...
private static function getConnection()
 ...
 private function getStaffDivisions(int $counselor_id): array
    {
        ...
        if (empty($divisions)) {
            $stmt2 = $conn->prepare("SELECT designation FROM users WHERE u_id = ? AND role = 'counselor' LIMIT 1");
            if ($stmt2) {
   ...
   public function getAllTicketsReport(string $start_date = '', string $end_date = '', string $status = '', string $priority = ''): array
    {
        $conn = self::getConnection();
        $counselor_id = $_SESSION['user']['u_id'];
        $divisions = [['did' => 10, 'name' => 'Counseling Unit']];
 ...
 */
public function getCounselorAssignmentReport(string $start_date = '', string $end_date = ''): array
{
    $conn = self::getConnection();
    $counselor_id = $_SESSION['user']['u_id'];
    $divisions = [['did' => 10, 'name' => 'Counseling Unit']];
     ...
     */
   public function getCounselorAssignmentSummary(string $start_date = '', string $end_date = ''): array
{
    $conn = self::getConnection();
    $counselor_id = $_SESSION['user']['u_id'];
    $divisions = [['did' => 10, 'name' => 'Counseling Unit']];
    ...
    $sql = "SELECT 
                COUNT(DISTINCT u.u_id) AS total_counselors, 
    ...
    $summary['avg_per_counselor'] = $summary['total_counselors'] 
        ? round($summary['total_assignments'] / $summary['total_counselors'], 1) 
        : 0;
