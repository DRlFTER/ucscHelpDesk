...
class StaffTicket
{
    ...
    public function getAllTickets(): array
    {
        ...
        try {
            ...
            // Base SELECT with assigned_level and is_overdue_pending flag
            ...
            if ($staff_level !== null && $staff_level <= 2) {
                // Levels 1-2: All in division OR assigned to self OR overdue pending in division
                $where = "(t.division IN ($placeholders) OR t.assigned_to = ?) OR (t.status = 'pending' AND t.created_at < DATE_SUB(NOW(), INTERVAL 3 DAY) AND t.division IN ($placeholders))";
                $sql = $baseSelect . " WHERE $where ORDER BY t.created_at DESC";
                
                $allParams = array_merge($didParams, [$staff_id], $didParams); // Divs + staff + divs for overdue
                ...
                // Levels 3+: Only assigned to self
