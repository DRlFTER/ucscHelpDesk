<?php
  ...
  $canCreate = in_array($roleName, ['staff', 'admin']);
  
  // Role-specific URL mapping
  $createUrl = ($roleName === 'staff') ? '/staff/staffAnnCreate' : "/{$roleName}/announcementCreate";
  $viewUrlBase = ($roleName === 'staff') ? '/staff/anView' : "/{$roleName}/announcement";
?>
<main>
    <div class="fullPage">
        <div class="pageLayout">
            <div class="ticketsFilters">
                ...
                <?php if ($canCreate): ?>
                <button id="newAnnBtn" class="btnWSvg" type="button" onclick="window.location.href='<?= htmlspecialchars($createUrl) ?>';">
...
window.ANNOUNCEMENTS_CONFIG = {
  ...
  announcementUrlBase: '<?= htmlspecialchars($viewUrlBase) ?>',
  canCreate: <?= $canCreate ? 'true' : 'false' ?>,
  canDelete: <?= $canCreate ? 'true' : 'false' ?>
};
</script>
<script src="/js/announcements/announcements.js"></script>
